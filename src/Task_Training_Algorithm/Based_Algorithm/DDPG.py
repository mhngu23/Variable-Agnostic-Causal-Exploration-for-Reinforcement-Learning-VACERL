...
class Actor(nn.Module):
    ...
    def forward(self, state, goal):
        return (self.actor(torch.cat([state, goal], 1)) * self.action_bounds) 
...
class Critic(nn.Module):
    ...
    def forward(self, state, action, goal):
        ...
        return -self.critic(torch.cat([state, goal, action], 1)) * self.H
    
class DDPG:
    def __init__(self, state_dim, action_dim, goal_dim, action_bounds=None, offset=None, lr=None, H=None):
